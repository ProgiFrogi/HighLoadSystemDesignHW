...
> Основа для задания 2 по курсу «Высоконагруженные системы». Решение опирается на требования из `requirements.md` и адаптировано под формат сдачи: `docs/architecture.md` + диаграммы + ADR.

## 1. Архитектурный стиль и обоснование

### Выбранный стиль

**Гибридная архитектура: Service-Based Architecture (SBA) + Event-Driven Architecture (EDA)**.

### Почему не чистый монолит

Монолит был бы дешевле в начале, но здесь есть несколько горячих и качественно разных контуров нагрузки:

- read-heavy каталог и поиск;
- критичный write-path заказа и оплаты;
- real-time tracking;
- фоновый fan-out уведомлений.

При пике порядка **35k RPS**, требовании быстро масштабироваться и жестком приоритете сохранности платежей монолит быстро станет компромиссом между скоростью разработки и управляемостью отказов.

### Почему не full microservices

Для команды из **5 человек** и ограниченного бюджета мелкозернистая MSA слишком дорога в эксплуатации: больше сетевых hop'ов, сложнее observability, выше цена ошибок в интеграции. На старте выгоднее держать **5 доменных сервисов крупнее**, а не десятки маленьких.

### Почему гибрид SBA + EDA подходит лучше всего

1. **Критичный пользовательский путь короткий и синхронный**: поиск -> заказ -> оплата.
2. **Некритичные побочные действия асинхронны**: уведомления, fan-out статусов, интеграции, reprocessing.
3. **Отказы локализуются**: проблема в Notification Service не должна блокировать оплату.
4. **Горизонтальное масштабирование простое**: все прикладные сервисы stateless.
5. **Реалистично для маленькой команды**: один managed PostgreSQL cluster, один Redis, одна шина событий.
...
Исходник для редактирования: [`diagrams/c4-context.mmd`](diagrams/c4-context.mmd)

### C4 Level 2 - Container Diagram

![C4 Level 2](diagrams/c4-container.png)

Исходник для редактирования: [`diagrams/c4-container.mmd`](diagrams/c4-container.mmd)
