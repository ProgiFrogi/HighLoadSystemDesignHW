...
**Обоснование:** в [requirements.md](requirements.md) для оплаты задано 99.99%, для каталога/поиска - 99.5%. Единая цифра «99.9%» для всей системы - компромисс: простой каталога на минуты не обнуляет выручку так, как простой оплаты, но пользователи ожидают стабильного приложения в пик. Ориентир по деньгам: при обороте порядка **100 млн руб/день** каждая минута простоя критичного контура ≈ **~70 тыс руб** недополученной выручки; **99.9%** ограничивает ожидаемый «видимый» простой до приемлемого для продукта уровня при ограниченном бюджете команды из 5 человек.
...
Оценка по модели из [архитектуры](architecture.md) и маппингу на сервисы YC. **Базовый сценарий** - суммы из [калькулятора Yandex Cloud](https://yandex.cloud/ru/prices) (регион **ru-central1**, конфигурации ниже). Сценарии **2×** и **5×** - экстраполяция масштабирования (увеличение vCPU/диска/числа нод).
...
| Managed PostgreSQL (primary + 3 replica, s4a-c2-m8 (2 vCPU, 8 ГБ RAM), 500 ГБ SSD) | 55 | 95 | 175 |
| Managed Redis (HA, 16 ГБ) | 10 | 20 | 50 |
| Managed Kafka (2 × s4a-c2-m8 (2 vCPU, 8 ГБ RAM), 32 ГБ SSD) | 25 | 40 | 85 |
| DNS + ALB + публичные IP | 5 | 8 | 12 |
| Диски брокеров Kafka / Cloud Logging | 5 | 8 | 15 |
| Monitoring + сетевой трафик | 15 | 22 | 35 |
