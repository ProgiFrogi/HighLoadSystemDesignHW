...
// OrderStatusResponse статус заказа
...
// validateMenuItems проверяет блюда через Catalog Service с Circuit Breaker
...
// createOrder создаёт заказ с идемпотентностью и outbox
func createOrder(w http.ResponseWriter, r *http.Request) {
	...
	// Проверяем идемпотентность (паттерн Idempotency Key)
	...
	// Проверяем Redis на наличие результата этого ключа (быстрый путь)
	...
	// Проверяем PostgreSQL на идемпотентность
	...
	if err == nil {
		// Заказ уже существует, сохраняем в Redis
	...
	// Валидируем блюда через Catalog Service
	...
	// Проверяем наличие всех блюд и считаем сумму
	totalAmount := 0
	...
	err = tx.QueryRow(ctx,
		...
		totalAmount,
		idemKey,
		fmt.Sprintf(`{"city":"%s","street":"%s"}`, 
			req.DeliveryAddr.City, req.DeliveryAddr.Street),
	).Scan(&orderID, &time.Time{})
	...
	// Сохраняем результат в Redis для быстрых повторных запросов
	order := OrderResponse{
		OrderID:     orderID,
		Status:      "created",
	...
	// Обновляем статус заказа в Redis для быстрого чтения
...
// getOrderStatus возвращает статус заказа (сначала Redis, потом БД)
func getOrderStatus(w http.ResponseWriter, r *http.Request) {
	orderID := chi.URLParam(r, "orderID")
	ctx := r.Context()

	// 1. Пробуем Redis (быстрый путь для статуса)
	status, err := rdb.Get(ctx, fmt.Sprintf("order_status:%s", orderID)).Result()
	if err == nil {
		// Есть в кеше, но нужны детали из БД
		w.Header().Set("X-Cache-Status", "HIT")
	}

	// 2. Получаем полные данные из PostgreSQL
...
// startOutboxWorker фоновый воркер для публикации событий из outbox
func startOutboxWorker() {
	...
	for {
		...
		for rows.Next() {
			...
			// В реальной системе здесь была бы отправка в RabbitMQ/Kafka
			// Для PoC просто логируем и помечаем как опубликованное
